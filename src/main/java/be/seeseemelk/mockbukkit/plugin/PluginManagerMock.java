...
public class PluginManagerMock implements PluginManager {
    ...
    private final Map<Plugin, List<AbstractListenerEntry>> eventListeners = new HashMap<>();
     ...
     * @param types       The array of parameter types the constructor must support. Note
     ...
     * @param types  The types of parameters that the constructor should be able to
     ...
     *
     * @param description The {@link PluginDescriptionFile} that contains information about
     *                    the plugin.
     * @param class1      The plugin to load.
     * @param parameters  Extra parameters to pass on to the plugin constructor. Must not be
     *                    {@code null}.
    ...
    public void callEvent(Event event) throws IllegalStateException {
        events.add(event);
        for (List<AbstractListenerEntry> listeners : eventListeners.values()) {
            for (AbstractListenerEntry entry : listeners) {
    ...
    public void registerEvents(Listener listener, Plugin plugin) {
        ...
        List<AbstractListenerEntry> listeners = eventListeners.get(plugin);
    ...
    public void registerEvent(Class<? extends Event> event, Listener listener, EventPriority priority,
        ...
        listeners.add(new EventExecutorListenerEntry(
                event, executor, listener
        ));
    }
